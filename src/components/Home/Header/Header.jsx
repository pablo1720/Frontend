...
const Header = ({ render, user, setSearchTerm, loginUserInfo }) => {
        ...
        <header className={`${style.header}`}>
            ...
            {windowWidth > 873 ? (
                <>
                    ...
                    {render || (user ? <UserInfo loginUserInfo={loginUserInfo} /> : <LogButtons />)}
                </>
            ) : (
                <div className={`${style.menuMovil} ${isMenuOpen ? style.menuOpen : ''}`}>
                    {render || <MainSearch setSearchTerm={setSearchTerm} user={user} />}
                    {render || (user ? (<> <ActionButtons /> <IndexNav /> <UserInfo loginUserInfo={loginUserInfo} /></>) : <LogButtons />)}
                </div>
            )}
